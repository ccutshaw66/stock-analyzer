...
async function main() {
  console.log("\nScanner 2.0 smoke test (3.5.5 — BB + ATR + RelVol + 52w + Gap + Earnings + Analyst)\n");
  ...
  if (scan.results.length) {
    ...
    check("row-signals-length", r0.signals.length >= 3 && r0.signals.length <= 7, `${r0.signals.length} signal results (expect 3-7, catalyst detectors skip if no FMP data)`);
  }
