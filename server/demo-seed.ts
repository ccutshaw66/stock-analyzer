/**
 * Demo account seeder — creates/resets the Otter Trader demo account.
 * Used by:
 *  1. `npm run seed:demo` (standalone)
 *  2. Server-side idle reset timer (imported from routes.ts)
 */

import pg from "pg";
import bcrypt from "bcryptjs";

export const DEMO_EMAIL = "ottertrader@stockotter.ai";
export const DEMO_PASSWORD = "demo123";
export const DEMO_DISPLAY_NAME = "Otter Trader";
export const DEMO_IDLE_TIMEOUT_MS = 60 * 60 * 1000; // 60 minutes

export async function seedDemoAccount(pool: pg.Pool): Promise<number> {
  ...
  try {
    ...
    await client.query(`DELETE FROM dividend_portfolio WHERE user_id = $1`, [userId]);
    ...
    const divPositions = [
      ["O", "Realty Income", 100, 55.20, "Monthly"],
      ["JEPI", "JPMorgan Equity Premium Income", 25, 56.80, "Monthly"],
      ["KO", "The Coca-Cola Company", 75, 58.40, "Quarterly"],
      ["JNJ", "Johnson & Johnson", 30, 155.00, "Quarterly"],
      ["XOM", "Exxon Mobil", 40, 108.00, "Quarterly"],
      ["T", "AT&T Inc.", 200, 17.50, "Quarterly"],
      ["VZ", "Verizon Communications", 100, 39.20, "Quarterly"],
      ["PG", "Procter & Gamble", 20, 162.00, "Quarterly"],
    ];
    for (const [ticker, name, shares, avgCost, freq] of divPositions) {
      await client.query(
        `INSERT INTO dividend_portfolio (user_id, ticker, company_name, shares, avg_cost, frequency, added_at) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [userId, ticker, name, shares, avgCost, freq, new Date().toISOString().split("T")[0]]
      );
    }
    ...
    // ═══════════════════════════════════════════════
    // SINGLE OPTIONS — Calls, Puts, Short Calls, Short Puts
    // ═══════════════════════════════════════════════
    ...
    trades.push({ pilotOrAdd: "Pilot", tradeDate: daysAgo(45), expiration: daysAgo(15), contractsShares: 5, symbol: "AMD", tradeType: "C", tradeCategory: "Option", strikes: "125", openPrice: -1.50, creditDebit: "DEBIT", commIn: 3.25, allocation: 750, closeDate: daysAgo(20), closePrice: 4.30, commOut: 3.25, behaviorTag: "Cashed Out for Coffee" });
    ...
    // ═══════════════════════════════════════════════
    // DSF (Double Spread Fly) — CDSF, PDSF
    // ═══════════════════════════════════════════════
    trades.push({ pilotOrAdd: "Pilot", tradeDate: daysAgo(5), expiration: daysFromNow(14), contractsShares: 3, symbol: "SPY", tradeType: "CDSF", tradeCategory: "Option", strikes: "560/565|565/570", openPrice: 0.45, spreadWidth: 5, creditDebit: "CREDIT", commIn: 7.80, allocation: 135 });
    trades.push({ pilotOrAdd: "Pilot", tradeDate: daysAgo(3), expiration: daysFromNow(21), contractsShares: 5, symbol: "QQQ", tradeType: "PDSF", tradeCategory: "Option", strikes: "470/475|475/480", openPrice: 0.55, spreadWidth: 5, creditDebit: "CREDIT", commIn: 13.00, allocation: 275 });
    trades.push({ pilotOrAdd: "Pilot", tradeDate: daysAgo(35), expiration: daysAgo(7), contractsShares: 5, symbol: "SPY", tradeType: "CDSF", tradeCategory: "Option", strikes: "555/560|560/565", openPrice: 0.50, spreadWidth: 5, creditDebit: "CREDIT", commIn: 13.00, allocation: 250, closeDate: daysAgo(10), closePrice: 0.10, commOut: 13.00, behaviorTag: "All to Plan" });
    trades.push({ pilotOrAdd: "Pilot", tradeDate: daysAgo(50), expiration: daysAgo(20), contractsShares: 3, symbol: "IWM", tradeType: "PDSF", tradeCategory: "Option", strikes: "205/210|210/215", openPrice: 0.40, spreadWidth: 5, creditDebit: "CREDIT", commIn: 7.80, allocation: 120, closeDate: daysAgo(25), closePrice: 0, commOut: 0, behaviorTag: "All to Plan" });
    trades.push({ pilotOrAdd: "Pilot", tradeDate: daysAgo(25), expiration: daysAgo(2), contractsShares: 5, symbol: "QQQ", tradeType: "CDSF", tradeCategory: "Option", strikes: "475/480|480/485", openPrice: 0.35, spreadWidth: 5, creditDebit: "CREDIT", commIn: 13.00, allocation: 175, closeDate: daysAgo(5), closePrice: 0.80, commOut: 13.00, behaviorTag: "Fear / Panic" });
    ...
    console.log(`[demo] Inserted ${trades.length} trades`);
