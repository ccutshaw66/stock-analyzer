...
import {
  ...
  Activity, AlertTriangle, Search, Loader2,
} from "lucide-react";
...
export default function Dividends() {
  ...
  const { data: scanResults, isLoading: isScanLoading } = useQuery<DividendData[]>({
    queryKey: ["/api/dividends/scan", scanTickers],
    queryFn: async () => {
      const url = scanTickers
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 space-y-6 max-w-[1200px] mx-auto" data-testid="dividends-page">
      ...
      <HelpBlock title="Understanding Dividends">
        ...
        <p><strong className="text-foreground">Ex-Dividend Date:</strong> You must own shares BEFORE this date to receive the upcoming dividend. Buying on or after this date means you won't get the next payment.</p>
      ...
      <div className="bg-card border border-card-border rounded-lg p-4" data-testid="dividend-scanner">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <button
            ...
            className="h-8 px-4 text-xs font-semibold rounded-md bg-primary text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
            data-testid="button-scan-default"
          >
            {isScanLoading && !scanTickers ? <Loader2 className="h-3 w-3 animate-spin" /> : <Search className="h-3 w-3" />}
            Scan Popular Dividend Stocks
          </button>
        ...
        {!scanTriggered && (
          <div className="flex flex-col items-center justify-center py-8 text-center bg-muted/20 border border-card-border/50 rounded-lg">
            ...
            <p className="text-xs text-muted-foreground font-medium">Click "Scan Popular Dividend Stocks" to get started</p>
            <p className="text-[10px] text-muted-foreground mt-1">
              Or enter custom tickers above to compare specific stocks.
            </p>
          </div>
        )}
      </div>
...
function MiniCard({ label, value, color, icon, subtitle }: {
