...
export default function TradeAnalysis() {
  const [, params] = useRoute("/trade/:ticker");
  const [inputTicker, setInputTicker] = useState("");
  const [activeTicker, setActiveTicker] = useState(params?.ticker?.toUpperCase() || "");

  const effectiveTicker = activeTicker || params?.ticker?.toUpperCase() || "";

  const { data, isLoading, error } = useQuery({
    queryKey: ["/api/trade-analysis", effectiveTicker],
    queryFn: async () => {
      if (!effectiveTicker) return null;
      const res = await apiRequest("GET", `/api/trade-analysis/${effectiveTicker}`);
      return res.json();
    },
        ...
        <header className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-foreground tracking-tight">Trade Analysis</h1>
            </div>
          </div>
          ...
          <form onSubmit={handleSearch} className="flex gap-2 max-w-md">
            ...
            />
            <Button type="submit" disabled={isLoading || !inputTicker.trim()} data-testid="button-analyze">
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4 text-center" data-testid="error-message">
            <p className="text-red-400 font-medium">
              {(error as Error).message || "Failed to load trade analysis."}
            </p>
          </div>
        )}
